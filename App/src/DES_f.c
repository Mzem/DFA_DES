...
//8 Sbox de 4 lignes et 16 colonnes
int Sbox[8][4][16] = {
	{
...
long f(long R, long Ki) 
{
	...
	for (int i = 0 ; i < 8 ; i++)
	{
		...
		//On ajoute les 4 bits provenant de la valeur du Sbox (qu'on décale à gauche pour mettre à la bonne position à chaque fois) au résultat 
		res = res | (Sbox[i][r][c] << (32 - 4 - i*4));
	}
	//Permutation pour obtenir un résultat en 32 bits
